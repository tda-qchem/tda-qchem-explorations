
## Resources and additional information

* [Prerequisites](https://tda-qchem.github.io/tda-qchem-examples/)

Quantum chemistry software:

* [`DIRAC`](http://www.diracprogram.org/)
    * [`DIRAC` tutorials](link)
    * literature: [@saue.etal_jcp_2020]
* [`AMS`]()
    * literature: [@
* [`GRASP`]()
    * literature: [@
* [PyADF]()
    * literature: [@
* [`Psi4`](https://github.com/psi4/psi4)
    * [`Psi4` tutorials](https://psicode.org/psi4manual/master/index_tutorials.html)
    * literature: [@smith.etal_jcp_2020]
* [`CREST`](https://github.com/crest-lab)
    * [`CREST` tutorials](https://crest-lab.github.io/crest-docs/page/examples)
    * literature: [@pracht.etal_jcp_2024]
* [ROSE]()
    * literature: [@

Additional scripts:

* [DensToolKit](https://github.com/jmsolano/denstoolkit)
    * literature: [@solano-altamirano.hernandez-perez_cpc_2015,@solano-altamirano.etal_tjocp_2024]

* [molden2aim.x](https://github.com/zorkzou/Molden2AIM.git)


Topological Data Analysis software:

* [TTK](https://topology-tool-kit.github.io/)
    * [`TTK` tutorials](https://topology-tool-kit.github.io/examples/)
    * literature: [@

Other tools:

* [qcten](TODO)

Additional resources:





